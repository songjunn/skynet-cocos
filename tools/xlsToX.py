...
jsonPath = os.path.abspath(os.path.join(os.path.dirname(__file__),"../"))+"\\assets\Script\config"
...
for root, dirs, files in os.walk( sourcePath ):
	for file in files:
		...
		if file.endswith('.xls'):
			saveToJs( os.path.join( root, file ), os.path.join( jsonPath, os.path.basename( file ).replace( '.xls', 'cfg.js' ) ) )
		elif file.endswith('.xlsx'):
			saveToJs( os.path.join( root, file ), os.path.join( jsonPath, os.path.basename( file ).replace( '.xlsx', 'cfg.js' ) ) )
